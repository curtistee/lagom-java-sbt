 ...
 * stores what the greeting should be (eg, "SayHello").
 * <p>
 * Event sourced entities are interacted with by sending them commands. This
 * entity supports two commands, a {@link UseGreetingMessage} command, which is
 * used to change the greeting, and a {@link SayHello} command, which is a read
...
public class HelloEntity extends PersistentEntity<HelloCommand, HelloEvent, HelloState> {
  ...
  public Behavior initialBehavior(Optional<HelloState> snapshotState) {
     ...
     * can be used.
     *
     * Otherwise, the default state is to use the SayHello greeting.
     */
    BehaviorBuilder b = newBehaviorBuilder(
        snapshotState.orElse(new HelloState("SayHello", LocalDateTime.now().toString())));
    ...
    ctx.thenPersist(new GreetingMessageChanged(entityId(), cmd.message),
        ...
        // Note we could respond with the message itself, e.g. "cmd.message"    
    ...
    /*
     * Command handler for the SayHello command.
     */
    b.setReadOnlyCommandHandler(SayHello.class,
